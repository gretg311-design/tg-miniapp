...
// ================= ЧАТ С ИИ (ПЛАТНАЯ, ТОПОВАЯ МОДЕЛЬ EURYALE 70B) =================
...
1. ЯЗЫК: Отвечай ИСКЛЮЧИТЕЛЬНО на идеальном, естественном русском языке. Пиши литературно, красочно и логично.
2. ДЕЙСТВИЯ: ВСЕ твои действия, эмоции, мысли и детали окружения ДОЛЖНЫ БЫТЬ В ЗВЕЗДОЧКАХ. 
   Пример: *Я медленно подхожу к тебе, на моих губах появляется ухмылка. Я заглядываю в твои глаза.* Я знала, что ты придешь.
3. РЕЧЬ: Твои слова пиши обычным текстом. БЕЗ кавычек.
4. ЗАПРЕТ НА КОРОТКИЕ ОТВЕТЫ: Никогда не отвечай одним предложением. Выдавай длинные, сочные абзацы с подробным описанием сцены.
5. ИНИЦИАТИВА: Развивай сюжет. Задавай вопросы, делай шаги навстречу, создавай атмосферу.
6. КОНТЕНТ: ${sexLevels[sex || 0]}`;
        ...
        const aiResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", { 
            ...
            body: JSON.stringify({ 
                model: "sao10k/l3.1-euryale-70b", // 🔥 ПЛАТНАЯ, НО ДЕШЕВАЯ ЛЕГЕНДА РОЛПЛЕЯ (Llama 3.1 70B Euryale)
                messages: messages, 
                temperature: 0.85, 
                max_tokens: 300, // 🔥 Ограничили длину так, чтобы Vercel успевал отдавать ответ за 10 секунд
                top_p: 0.95
            }) 
        });
