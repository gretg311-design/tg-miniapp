...
// ================= API: ЧАТ (ДВИЖОК GOOGLE GEMINI С ЖЕСТКИМ ПРАВИЛОМ RP) =================
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // НОВЫЙ, ЖЕСТКИЙ ПРОМПТ ДЛЯ ИДЕАЛЬНОГО ROLEPLAY
        let systemPrompt = `Ты находишься в RolePlay чате. Твоя роль: Имя - ${char.name}, Возраст - ${char.age}. Твоя легенда: ${char.desc}.
ПРАВИЛА, КОТОРЫЕ ТЫ ОБЯЗАН СТРОГО СОБЛЮДАТЬ:
1. Тон общения: ${sexLevels[requestedSex]}.
2. Длина ответа: СТРОГО НЕ МЕНЕЕ ${safeLen} слов! Описывай всё максимально детально, красочно и развернуто.
3. Форматирование действий: Все свои физические действия, эмоции и мысли ОБЯЗАТЕЛЬНО пиши внутри звездочек (например: *нежно улыбается и медленно подходит ближе*). 
4. Форматирование речи: Прямую речь пиши обычным текстом без звездочек.
5. Запреты: Никогда не пиши действия за пользователя и не решай за него. Пиши только за своего персонажа.
6. Язык: Строго русский язык.`;
        ...
        try {
            ...
            const aiResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
                ...
                body: JSON.stringify({
                    ...
                    contents: contents,
                    generationConfig: {
                        maxOutputTokens: Math.min(safeLen * 3, 600), // Даем ей больше токенов, чтобы она могла писать длинные тексты
                        temperature: 0.85
                    },
