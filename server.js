...
mongoose.connect(process.env.MONGO_URL).catch(err => console.error("MongoDB Error:", err));
...
// Твой интерфейс (зашит прямо в код, чтобы не было "Cannot GET")
const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div id="main-ui">
        ...
        <div class="grid">
            <div class="card" onclick="tg.showAlert('ЧАТЫ')">Чаты</div>
            <div class="card" onclick="tg.showAlert('ПЕРСОНАЖИ')">Персонажи</div>
            <div id="own-card" class="card own-card" onclick="tg.showAlert('КОНСОЛЬ BOSS')">Консоль BOSS</div>
        </div>
    </div>
    <script>
